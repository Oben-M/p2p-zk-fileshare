...
const SIZES_MB = [10, 100]; // 10 MB, 100 MB
...
async function runOnce(filePath) {
  ...
  return { uploadMs, downloadMs, totalMs: uploadMs + downloadMs, bytes: buf.length };
}
...
async function main() {
  ...
  for (const sizeMb of SIZES_MB) {
    ...
    for (let i = 1; i <= RUNS_PER_SIZE; i++) {
      ...
      try {
        const { uploadMs, downloadMs, totalMs, bytes } = await runOnce(path);
        totals.push(totalMs);
